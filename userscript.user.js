...
class MutationObserverRegistry {
    ...
    addFunctionForAnyMutation = functionRef => this.functionsForAnyMutation.push(functionRef);
    addFunctionForNodeAddition = functionRef => this.functionsForNodeAddition.push(functionRef);
...
class ConditionalFormatting {
    constructor() {
        this.textMatchStyles = JSON.parse(GLOBAL.settingsManager.settings.textMatchStyles);
    ...
    run = () => {
        GLOBAL.mutationObserverRegistry.addFunctionForAnyMutation(this.processMutation);
        GLOBAL.mutationObserverRegistry.addFunctionForNodeAddition(this.processTextNode);
        GLOBAL.mutationObserverRegistry.addFunctionForNodeAddition(this.defectRequestMod);
    }
    ...
    applyFeedFont = mutation => {
        if (GLOBAL.settingsManager.settings.overrideStandardFeedFont) {
            ...
            const fontToApply = GLOBAL.settingsManager.settings.customFeedFont.length > 0 ? GLOBAL.settingsManager.settings.customFeedFont : GLOBAL.settingsManager.settings.feedFontFamily;
            if (mutationTarget.attr('id') == 'emailuiFrame') {
                ...
                emailExpandedBody.css({ "fontFamily": fontToApply, "fontSize": `${GLOBAL.settingsManager.settings.feedFontSize}px` });
            } else if (mutationTarget.hasClass('cuf-compactBody')) {
                let internalPostExpandedTextSpan = mutationTarget.find('.feedBodyInner');
                if (internalPostExpandedTextSpan.length > 0) internalPostExpandedTextSpan.css({ "fontFamily": fontToApply, "fontSize": `${GLOBAL.settingsManager.settings.feedFontSize}px` });
            } else if (mutationTarget.hasClass('cuf-element')) {
                let collapsedFeedTextSpan = mutationTarget.find(".preamble_custom-summary");
                if (collapsedFeedTextSpan.length > 0) {
                    collapsedFeedTextSpan.css({ "fontFamily": fontToApply, "fontSize": `${GLOBAL.settingsManager.settings.feedFontSize}px` });
                }
                let expandedEmailTextSpan = mutationTarget.find('.uiOutputText');
                if (expandedEmailTextSpan.length > 0) expandedEmailTextSpan.css({ "fontFamily": fontToApply, "fontSize": `${GLOBAL.settingsManager.settings.feedFontSize}px` });
            }
        }
    }
    ...
    listenForAttachedImages = mutation => {
        ...
        if (mutationTarget.hasClass('forceContentModalPreviewPlayer')) {
            GLOBAL.imageContainerDiv = mutationTarget.find('div.thumbnail img.pageImg').parent();
            let imageUrl = mutationTarget.find('div.thumbnail img.pageImg').attr('src');
    ...
    expandT3investigationActionTextInput = mutation => {
        const mutationTarget = $(mutation.target);
        if (mutationTarget.hasClass("ql-editor slds-rich-text-area__content slds-text-color_weak slds-grow")) {
            mutationTarget.css({ "maxHeight": '10000px' });
            let internalPostOuterBox = mutationTarget.closest(".forceChatterMessageBodyInputRichTextEditor");
    ...
    expandEmailInput = mutation => {
        ...
        if (mutationTarget.hasClass("uiInput")) {
            ...
            if (firstIframe.length > 0) {
                ...
                const divForResizing = $(firstIframe[0]).contents().find(selectorForResizing);
    ...
    tampermonkeyLinkInQuickText = body => {
        ...
        if (/\&lt;tampermonkeylink\&gt;.*?\&lt;\/tampermonkeylink\&gt;/.test(bodyText)) {
            bodyText = bodyText.replace(/\&lt;tampermonkeylink\&gt;/g, '<a href="');
            bodyText = bodyText.replace(/\&lt;\/tampermonkeylink\&gt;/g, '">Tampermonkey link</a>');
            body.html(bodyText);
        }
    }
    ...
    tryToFindFileLinks = () => {
        ...
        let fileLinks = $(`a:not('.tabHeader')[href*="/ContentDocument/"] span.itemTitle.desktop`)
        ...
        if (window.location.pathname != GLOBAL.pathname) {
            ...
            if (pathNameArray.length > 4) {
                if (pathNameArray[3].toLowerCase() == "case") {
                    if (GLOBAL.activeCaseId != pathNameArray[4] && GLOBAL.activeCaseId) GLOBAL.fileLinks = null;
            ...
            if (Object.keys(GLOBAL.caseIdNumberPairs).includes(GLOBAL.activeCaseId)) {
                GLOBAL.activeCaseNumber = GLOBAL.caseIdNumberPairs[GLOBAL.activeCaseId].caseNumber;
    ...
    highlightActiveCase = () => {
        let highlightedAlready = GLOBAL.caseIdNumberPairs[GLOBAL.activeCaseNumber].highlighted;
        if (!highlightedAlready) {
            ...
            $(".conditional-formatting-highlighted-case").removeClass(".conditional-formatting-highlighted-case");
            let activeCaseTextNode = GLOBAL.caseIdNumberPairs[GLOBAL.activeCaseNumber].textNode;
            let itemToHighlight = $(activeCaseTextNode).closest(".slds-split-view__list-item");
            if (itemToHighlight.length > 0) {
                let style = this.getStyleBasedOnTextArray(this.getTextFromNode(itemToHighlight.get(0)));
                if (style) {
    ...
    getTextFromNode = node => {
        let text = [], textNode, walker = document.createTreeWalker(node, NodeFilter.SHOW_TEXT, null, false);
    ...
    processTextNode = node => {
        const start = Date.now();
        let textNode, walker = document.createTreeWalker(node, NodeFilter.SHOW_TEXT, null, false);
        while (textNode = walker.nextNode()) {
            this.clickOnInternalOnly(textNode);
            this.color(textNode);
            this.gatherCaseIdNumberPairs(textNode);
            if (GLOBAL.settingsManager.settings.allUrlHyperlinkingBeta) this.allUrlHyperlinking(textNode);
    ...
    generalMessageIsNullHighlighting = textNode => {
        if (textNode.textContent == "General Message") {
            const parentToHighlight = $(textNode.parentNode).closest('div.slds-form-element').get(0);
            const shadowRoot = parentToHighlight.parentNode.parentNode.host.shadowRoot;
    ...
    closeTheAccessLinkRequestPopup = textNode => {
        if (textNode.textContent == "Access Link Request") {
            let button = $(textNode.parentNode).closest("div.modal-container").find('button[title="Close this window"]');
            if (button.length > 0) {
                button.click();
            }
        }
    ...
    tryToHighlightSupportScoreInHighlightsPanel = textNode => {
        this.supportScoreHighlight($(textNode.parentNode.parentNode), this.querySelectorDeep(`slot`, textNode.parentNode.parentNode)?.[0]?.innerText);
    }
    ...
    findEscalatedToT3OrDevWarning = textNode => {
        if (textNode.textContent == "WARNING!!! This case has already been escalated to Tier 3 or Dev. Do not click save!") {
    ...
    waitForFileLinks = () => {
        setTimeout(() => {
            ...
            if (fileLinks.length > 0) {
                ...
                if (typeof GLOBAL.imageUrlResolvedCallback == "function") GLOBAL.imageUrlResolvedCallback(fileLinks);
            } else this.waitForFileLinks();
        }, 100);
    }

    allUrlHyperlinking = textNode => {
        if (/^(http|ftp|https):\/\/([\w_-]+(?:(?:\.[\w_-]+)+))([\w.,@?^=%&:\/~+#-]*[\w@?^=%&\/~+#-])?$/.test(textNode.textContent)) {
            if (!$(textNode.parentNode).is('a')) {
                let url = textNode.textContent;
                let label = /^https:\/\/clarizenint\.atlassian\.net\/browse\/CLZDEV-\d*/.test(url) ? url.split("/").pop() : url;
                const link = document.createElement('a');
                link.target = "_blank";
                link.href = url;
                link.textContent = label;
                textNode.parentNode.replaceChild(link, textNode);
            } else if (/^((https|http|HTTPS|HTTP):\/\/.*(EXP|exp).*(CSIG|csig)[-A-Za-z0-9+&@#\/%?=~_|!:,.;]+[-A-Za-z0-9+&@#\/%=~_|])$/g.test(textNode.textContent)) {
                const accessLinkAnchor = $(textNode.parentNode);
                const accessLink = accessLinkAnchor.attr('href');
                const accessLinkExpirationCalculation = this.accessLinkExpiration(accessLink);
    ...
    tasktopZendeskIdHyperlinking = textNode => {
        if (/^TT-\d\d*\d$/.test(textNode.textContent)) {
            if (!$(textNode.parentNode).is('a')) {
                ...
                setTimeout(() => {
                    $(textNode.parentNode).html(`<a href="${url}" target="_blank">${textNode.textContent}</a>`);
                }, 100);
            }
        }
    }
    ...
    tasktopJiraIdHyperlinking = textNode => {
        if (/^(VIZ|APPS|CON|FC|ROOT|PVC|viz|con|apps|fc|root|pvc)\-\d*$/.test(textNode.textContent)) {
            if (!$(textNode.parentNode).is('a')) {
                let id = textNode.textContent;
    ...
    tasktopJiraIdHyperlinkingManyIds = textNode => {
        if (/^((VIZ|APPS|CON|FC|ROOT|PVC|viz|con|apps|fc|root|pvc)\-\d*\,).*(VIZ|APPS|CON|FC|ROOT|PVC|viz|con|apps|fc|root|pvc)\-\d*$/.test(textNode.textContent)) {
            if (!$(textNode.parentNode).is('a')) {
                let ids = textNode.textContent.replaceAll(" ", "").replaceAll(',', '%2c');
    ...
    sciformaJiraIdHyperlinking = textNode => {
        if (/^(MIL|mil|SCI|sci|KI|ki|DELIVER|deliver|Deliver|VAN|van|INT|int|OPS|ops)\-\d*$/.test(textNode.textContent)) {
            if (!$(textNode.parentNode).is('a')) {
                let id = textNode.textContent;
    ...
    sciformaJiraIdHyperlinkingManyIds = textNode => {
        if (/^((VIZ|APPS|CON|FC|OPS|ROOT|PVC|viz|con|apps|fc|ops|root|pvc)\-\d*\,).*(VIZ|APPS|CON|FC|OPS|ROOT|PVC|viz|con|apps|fc|ops|root|pvc)\-\d*$/.test(textNode.textContent)) {
            if (!$(textNode.parentNode).is('a')) {
                let ids = textNode.textContent.replaceAll(" ", "").replaceAll(',', '%2c');
    ...
    supportScoreTd = el => {
        ...
        let headerIndex = el.closest("table").find(`span[data-tooltip="Support Score"]`).closest("th").attr("data-column-index");
    ...
    supportScoreInTableHighlighting = textNode => {
        ...
        const offspringOfTdToHighlight = textNode.getRootNode()?.host?.parentNode?.host?.parentNode?.parentNode;
        if (!offspringOfTdToHighlight) return;
        const elToHighlight = $(offspringOfTdToHighlight).closest(`td[data-label="Support Score"]`);
        ...
        if (this.textValuesToMonitor.includes(textNode.textContent)) {
            ...
            if (table.length > 0) {
                ...
                if (th1.length > 0) {
                    ...
                    let spans = table.find(`td:nth-child(${headerIndex}) > span.slds-grid.slds-grid_align-spread > span.slds-truncate`);
                    spans.each((idx, el) => {
    ...
    clickOnInternalOnly = textNode => {
        if (textNode.textContent == "Internal Only") {
            if (JSON.parse(GLOBAL.settingsManager.settings.autoClickInternalUpdatesTab)) $(textNode.parentNode).click();
        }
    }
    ...
    color = textNode => {
        for (let i in this.textMatchStyles) {
            let style = this.textMatchStyles[i];
            if (style.exactMatch && style.text == textNode.textContent || !style.exactMatch && textNode.textContent.includes(style.text)) {
                if (style.ancestorToHighlight && Array.isArray(style.ancestorToHighlight)) {
                    ...
                    if (!ancestorFound && style.highlightIfAncestorNotFound && textNode.parentNode.nodeType === Node.DOCUMENT_FRAGMENT_NODE) {
                        const fragmentObserver = new MutationObserver(this.textMutationCallback);
                        fragmentObserver.observe(textNode, { subtree: false, childList: false, attributes: false, characterData: true });
    ...
    gatherCaseIdNumberPairs = textNode => {
        if (/^\d{8}$/.test(textNode.textContent) && $(textNode.parentNode).hasClass('uiOutputText') && $(textNode.parentNode.parentNode).hasClass('test-splitViewCardData')) {
            let caseId = $(textNode.parentNode).closest('a').attr('data-recordid');
            GLOBAL.caseIdNumberPairs[textNode.textContent] = { caseNumber: textNode.textContent, caseId: caseId, textNode: textNode, highlighted: false };
            GLOBAL.caseIdNumberPairs[caseId] = { caseNumber: textNode.textContent, caseId: caseId, textNode: textNode, highlighted: false };
            this.monitorActiveCasesAsTheyAppear();
        }
    }

    monitorActiveCasesAsTheyAppear = () => {
        GLOBAL.activeCaseId = GLOBAL.pathname.split('/')[GLOBAL.pathname.split('/').length - 2];
        if (Object.keys(GLOBAL.caseIdNumberPairs).includes(GLOBAL.activeCaseId)) {
            GLOBAL.activeCaseNumber = GLOBAL.caseIdNumberPairs[GLOBAL.activeCaseId].caseNumber;
            this.highlightActiveCase();
        }
    ...
    defectRequestMod = node => {
        ...
        const results = $(node).find(selector).filter(function () { return $(this).text() == selectorText; });
        if (results.length > 0) {
            const fieldContent = results.parent().siblings().find('span span');
            if (fieldContent.length > 0) {
                let potentialUrl = fieldContent.html();
                if (/^http.*\:\/\/.*\./.test(potentialUrl) && !potentialUrl.includes(' ')) {
                    let label = /^https:\/\/clarizenint\.atlassian\.net\/browse\/CLZDEV-\d*/.test(potentialUrl) ? potentialUrl.split("/").pop() : potentialUrl;
                    fieldContent.html(`<a href='${potentialUrl}' target='_blank'>${label}</a>`);
                }
            }
        }
    }
    ...
    accessLinkExpiration = accessLink => {
        ...
        const expirationStamp = Number(accessLink.substring(expPos + searchTerm.length, expPos + searchTerm.length + 14));
        const expirationDate = new Date(new Date(expirationStamp).setFullYear(new Date(expirationStamp).getFullYear() - 1969));
        const now = new Date();
        let label, color = "red", expired = false;
...
class WhosLooking {
    ...
    check = () => {
        setTimeout(() => {
            if (GLOBAL.settingsManager.settings.userName && GLOBAL.activeCaseId && GLOBAL.mouseMovedOn && (GLOBAL.activeCaseId != this.caseIdSent || GLOBAL.mouseMovedOn != this.lastMouseActivitySent || GLOBAL.keyPressedOn != this.lastKeyboardActivitySent)) { // GLOBAL.userName
                let lastActivityTimestamp = GLOBAL.keyPressedOn ? (GLOBAL.keyPressedOn > GLOBAL.mouseMovedOn ? GLOBAL.keyPressedOn : GLOBAL.mouseMovedOn) : GLOBAL.mouseMovedOn;
                this.payload = {
    ...
    updateUI = response => {
        if (response.status == 200) {
            GLOBAL.whosLookingResponse = { response: JSON.parse(response.response), timestamp: Date.now() };
            ...
            console.log('API error detected. Please get in touch with rpumputis@planview.com for support.');
        }
    }
}
...
class SettingsManager {
    constructor() {
        ...
        this.defaultSettings = {
            textMatchStyles: JSON.stringify([
                { text: "New", exactMatch: true, bgColor: "#ffd631ff", textColor: "#ffffff", ancestorToHighlight: [], highlightIfAncestorNotFound: true },
                { text: "Active", exactMatch: true, bgColor: "#e33232ff", textColor: "#ffffff", ancestorToHighlight: [], highlightIfAncestorNotFound: true },
                { text: "Pending Internal Team Response", exactMatch: true, bgColor: "#de6e31ff", textColor: "#ffffff", ancestorToHighlight: [], highlightIfAncestorNotFound: true },
                { text: "In Tier 3 Support", exactMatch: true, bgColor: "#de6e31ff", textColor: "#ffffff", ancestorToHighlight: [], highlightIfAncestorNotFound: true },
                { text: "In Dev", exactMatch: true, bgColor: "#90ba92ff", textColor: "#ffffff", ancestorToHighlight: [], highlightIfAncestorNotFound: true },
                { text: "In Dev - Release Scheduled", exactMatch: true, bgColor: "#34b738ff", textColor: "#ffffff", ancestorToHighlight: [], highlightIfAncestorNotFound: true },
                { text: "In Dev - Review", exactMatch: true, bgColor: "#90ba92ff", textColor: "#ffffff", ancestorToHighlight: [], highlightIfAncestorNotFound: true },
                { text: "In Cloud Ops", exactMatch: true, bgColor: "#82c1beff", textColor: "#ffffff", ancestorToHighlight: [], highlightIfAncestorNotFound: true },
                { text: "In Cloud Ops - Scheduled", exactMatch: true, bgColor: "#1dbbb3ff", textColor: "#ffffff", ancestorToHighlight: [], highlightIfAncestorNotFound: true },
                { text: "In Cloud Ops - Pending Build", exactMatch: true, bgColor: "#1dbbb3ff", textColor: "#ffffff", ancestorToHighlight: [], highlightIfAncestorNotFound: true },
                { text: "Pending Customer", exactMatch: true, bgColor: "#3091EC", textColor: "#ffffff", ancestorToHighlight: [], highlightIfAncestorNotFound: true },
                { text: "On Hold", exactMatch: true, bgColor: "#2F3941", textColor: "#ffffff", ancestorToHighlight: [], highlightIfAncestorNotFound: true },
                { text: "Case Closed", exactMatch: true, bgColor: "#eaedf0ff", textColor: "#000", ancestorToHighlight: [], highlightIfAncestorNotFound: true },
                { text: "Closed", exactMatch: true, bgColor: "#eaedf0ff", textColor: "#000", ancestorToHighlight: [], highlightIfAncestorNotFound: true },
                { text: "Merged", exactMatch: true, bgColor: "#eaedf0ff", textColor: "#000", ancestorToHighlight: [], highlightIfAncestorNotFound: true },
                { text: "Ongoing Activity", exactMatch: true, bgColor: "#3ba755", textColor: "#ffffff", ancestorToHighlight: [], highlightIfAncestorNotFound: true },
                { text: "Solution Provided", exactMatch: true, bgColor: "#2F3941", textColor: "#D8DCDE", ancestorToHighlight: [], highlightIfAncestorNotFound: true },
                { text: "1 - Critical", exactMatch: true, bgColor: "#f30707ff", textColor: "#ffffff", ancestorToHighlight: [".slds-form-element"], highlightIfAncestorNotFound: true },
                { text: "2 - Major", exactMatch: true, bgColor: "#e1590fff", textColor: "#ffffff", ancestorToHighlight: [".slds-form-element"], highlightIfAncestorNotFound: true },
                { text: "3 - Moderate", exactMatch: true, bgColor: "#FBC02D", textColor: "#333333", ancestorToHighlight: [".slds-form-element"], highlightIfAncestorNotFound: true },
                { text: "4 - Minor", exactMatch: true, bgColor: "#d9ead3", textColor: "#333333", ancestorToHighlight: [".slds-form-element"], highlightIfAncestorNotFound: true },
                { text: "1 - Flagged to Case Owner", exactMatch: true, bgColor: "#d9ead3", textColor: "#000000", ancestorToHighlight: [".slds-form-element"], highlightIfAncestorNotFound: true },
                { text: "1 - Watching", exactMatch: true, bgColor: "#d9ead3", textColor: "#000000", ancestorToHighlight: [".slds-form-element"], highlightIfAncestorNotFound: true },
                { text: "2 - Elevated to Manager", exactMatch: true, bgColor: "#ffe599", textColor: "#000000", ancestorToHighlight: [".slds-form-element"], highlightIfAncestorNotFound: true },
                { text: "3 - Escalated to Development", exactMatch: true, bgColor: "#f5b799", textColor: "#000000", ancestorToHighlight: [".slds-form-element"], highlightIfAncestorNotFound: true },
                { text: "3 - Escalated to PM/Infrastructure team", exactMatch: true, bgColor: "#f5b799", textColor: "#000000", ancestorToHighlight: [".slds-form-element"], highlightIfAncestorNotFound: true },
                { text: "4 - Global XCAR", exactMatch: true, bgColor: "#ea9999", textColor: "#000000", ancestorToHighlight: [".slds-form-element"], highlightIfAncestorNotFound: true },
                { text: "To: Internal", exactMatch: true, bgColor: "#FFF6D9", textColor: "", ancestorToHighlight: [".cuf-element"], highlightIfAncestorNotFound: false },
                { text: "Case status updated", exactMatch: true, bgColor: "#FFF6D9", textColor: "", ancestorToHighlight: [".cuf-element"], highlightIfAncestorNotFound: false },
                { text: "Case updated", exactMatch: true, bgColor: "#FFF6D9", textColor: "", ancestorToHighlight: [".cuf-element"], highlightIfAncestorNotFound: false },
                { text: "Case created", exactMatch: true, bgColor: "#FFF6D9", textColor: "", ancestorToHighlight: [".cuf-element"], highlightIfAncestorNotFound: false },
                { text: "...", exactMatch: true, bgColor: "#FFF6D9", textColor: "", ancestorToHighlight: [".cuf-element"], highlightIfAncestorNotFound: false },
                { text: "Elite", exactMatch: false, bgColor: "#ffcb2a", textColor: "", ancestorToHighlight: [".slds-form-element"], highlightIfAncestorNotFound: false },
                { text: "Premium", exactMatch: false, bgColor: "#bababa", textColor: "", ancestorToHighlight: [".slds-form-element"], highlightIfAncestorNotFound: false },
                { text: "GREEN", exactMatch: true, bgColor: "#d9ead3", textColor: "#000000", ancestorToHighlight: [".slds-form-element"], highlightIfAncestorNotFound: true },
                { text: "AMBER", exactMatch: true, bgColor: "#f5b799", textColor: "#000000", ancestorToHighlight: [".slds-form-element"], highlightIfAncestorNotFound: true },
                { text: "RED", exactMatch: true, bgColor: "#ea9999", textColor: "#000000", ancestorToHighlight: [".slds-form-element"], highlightIfAncestorNotFound: true },
                { text: "Scheduled Task", exactMatch: true, bgColor: "#3ba755", textColor: "#ffffff", ancestorToHighlight: [], highlightIfAncestorNotFound: true }
            ]),
            avatarColor: "#E34F32",
    ...
    applyDefaultSettings = () => {
        if (true) { //GLOBAL.version == "v20251211" && this.settings["clearedTextMatchStylesInVersion"] != "v20251211"
            this.update("clearedTextMatchStylesInVersion", "v20251211");
            this.update("textMatchStyles", JSON.stringify(JSON.parse(this.defaultSettings["textMatchStyles"]), undefined, 4));
...
class UIinjector {
    ...
    showWhosLooking = () => {
        setTimeout(() => {
            ...
            if (GLOBAL.whosLookingResponse) { // && this.whosLookingResponseTimestamp != GLOBAL.whosLookingResponse.timestamp
    ...
    injectWhosLookingIndicator = () => {
        ...
        for (let i in GLOBAL.whosLookingResponse.response) {
            ...
            let secondsAgo = ~~(Date.now() / 1000) - ~~(record.lastActivityTimestamp / 1000);
            let secondsTo20percentOpacity = 100;
            let secondsToShow = 300;
            let opacity = 1 - 0.8 * (secondsAgo / secondsTo20percentOpacity < 1 ? secondsAgo / secondsTo20percentOpacity : 1);
            feedActions.prepend(this.whosLookingIndicatorTemplate({
    ...
    whosLookingIndicatorTemplate = settings => {
        return (settings.show ? `<div class='roland-ui-mods-whoslooking-indicator' style='opacity: ${settings.opacity}; background-color: ${settings.backgroundColor}' title='${settings.title}'>
    ...
    injectSettingsButton = retrying => {
        ...
        if (headerFirstChild.length > 0) {
            ...
            $('div.slds-global-header > div:nth-child(1)').after(this.headerButtonsTemplate());
        } else {
            if (!retrying) {
                setTimeout(() => {
    ...
    uiEvents = () => {
        $(document).ready(() => {
            $(document).off('click', '#roland-ui-mods-settings-button').on('click', '#roland-ui-mods-settings-button', e => {
                this.settingsButtonClicked(e);
            });
            $(document).off('click', '#roland-ui-mods-timesheets-button').on('click', '#roland-ui-mods-timesheets-button', e => {
                window.open('https://tam-04a6caab6366.herokuapp.com/timesheets', '_blank');
            });
            $(document).off("click", ".tabs__item:not(.roland-ui-mods-chat-button-general)").on("click", ".tabs__item:not(.roland-ui-mods-chat-button-general)", e => {
                const chatTopContainer = $(e.target).closest("div.forceChatterStyle");
                const chatToModify = chatTopContainer.find("div.cuf-feed");
                chatToModify.show();
                $(`div.roland-ui-mods-chat-container-${GLOBAL.activeCaseId}`).remove();
            });
            $(document).off('click', '#feed-font-selector').on('click', '#feed-font-selector', e => {
    ...
    keyPress = e => {
        ...
        if (evtobj.keyCode == 73 && evtobj.ctrlKey) {
            if (GLOBAL.settingsManager.settings.openAllImagesUsingCtrlI) {
                let viewAllButton = $(`a[href*="AttachedContentDocuments"] .view-all-label`);
                if (GLOBAL.fileLinks) {
                    this.openImages();
    ...
    openImages = async () => {
        ...
        for (let i in buttonsArr) {
            ...
            let closeButton = $('div.forceContentBasePreviewToolbar button[title="Close"]');
            closeButton.click();
        }
        let imagesHtml = this.addImagesToDiv(GLOBAL.imageContainerDiv, GLOBAL.attachedImageUrls);
    ...
    showImagePopup = imagesHtml => {
        ...
        $("body").append(`<div id="roland-ui-mods-image-viewer-popup" style="padding:5px;position:fixed;width:90%;height:90%;top:5%;left:5%;z-index:9000;"></div>`);
    ...
    closeTabs = () => {
        let buttons = $(`button.slds-button.slds-button_icon.slds-button_icon-x-small.slds-button_icon-container[title^="Close "]`);
        if (buttons.length > 0) {
            buttons.click();
        }
    }
    ...
    settingsButtonClicked = async e => {
        ...
        $("body").append(`<div id="roland-ui-mods-settings-popup" style="padding:5px;position:fixed;width:70%;height:70%;top:15%;left:15%;z-index:9000;"></div>`);
...
const rolandUImods = () => {
    ...
    GLOBAL.mutationObserverRegistry.observer.observe(document.body, { subtree: true, childList: true, attributes: true, characterData: true }); //, characterData: true,attributes: true childList: true,
    ...
    if (!isNaN(parseFloat(GLOBAL.settingsManager.settings.listRefreshRate))) {
        setTimeout(() => {
            if (!GLOBAL.settingsManager.settings.disableListRefresh) {
                let button = $($("button", $("force-list-view-manager-button-bar"))[0]);
                if (button.length > 0) {
                    button.click();
                }
            }
...
const refreshReports = () => {
    if (!isNaN(parseFloat(GLOBAL.settingsManager.settings.reportRefreshRate)) && GLOBAL.settingsManager.settings.reportRefreshRate > 0) {
        setTimeout(() => {
...
const detectUserName = node => {
    ...
    if (results.length > 0) {
        if (GLOBAL.profileButton != null && GLOBAL.settingsManager.settings.userName == null) {//GLOBAL.userName
...
const tryOpeningProfilePopupToLookForName = () => {
    if (GLOBAL.settingsManager.settings.userName == null) { //GLOBAL.userName
        GLOBAL.profileButton.click();
        setTimeout(() => {
